...
early_stopping_patience=30 #Relu
#early_stopping_patience=8 #Softplus
...
def train_model(model, train_loader, val_loader, test_loader, criterion, optimizer, num_epochs,log_file_path):
    ...
    #scheduler = lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.1, patience=10) #Relu 
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), './Saved_Model/Eurosat_soft_144_164_4.pth')
    ...
    # Load the best model
    model.load_state_dict(torch.load('./Saved_Model/Eurosat_soft_144_164_4.pth'))
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = FrequencyConv2D(13, 64, kernel_size=64, apply_downsample=False)
...
train_path = './EuroSAT1/train'
val_path = './EuroSAT1/val'
test_path = './EuroSAT1/test'
...
# Define the transform for training and validation datasets
'''train_val_transform = transforms.Compose([
    transforms.RandomRotation(degrees=15),  # Moderate rotation to preserve spatial orientation
    transforms.RandomHorizontalFlip(),      # Satellite images often benefit from flipping
    transforms.RandomVerticalFlip(),        # Vertical flipping can simulate different perspectives
    #transforms.RandomResizedCrop(size=(64, 64), scale=(0.9, 1.0)),  # Slight cropping to focus on key regions
    #transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.2, hue=0.05),  # Subtle color variations
    #transforms.GaussianBlur(kernel_size=3), # Add slight blur for robustness
    #transforms.ToTensor(),
    #transforms.Normalize(mean=mean, std=std)
    #transforms.Normalize(mean=[0.485,0.456, 0.406],std=[0.229, 0.224, 0.225])
])'''

# Test transformations: minimal changes to ensure consistency
'''test_transform = transforms.Compose([
    #transforms.Resize((64, 64)),            # Standardize test image size
    #transforms.ToTensor(),
    #transforms.Normalize(mean=mean, std=std)
    #transforms.Normalize(mean=[0.485,0.456, 0.406],std=[0.229, 0.224, 0.225])
])'''
...
class CustomTransforms:
    def __init__(self, rotation=15, hflip_prob=0.5, vflip_prob=0.5):
    ...
    def __call__(self, img):
        ...
        angle = torch.empty(1).uniform_(-self.rotation, self.rotation).item()
...
class FFTDataset(Dataset):
    def __init__(self, root_dir, transform=None, target_size=(64, 64)):
        ...
        Args:
            ...
            target_size (tuple): Resize target dimensions (default 64x64).
        """
        ...
        for cls_name in self.classes:
            ...
            for img_name in os.listdir(cls_folder):
                if img_name.endswith('.tif'):  # Only include .tif files
    ...
    def __getitem__(self, idx):
        ...
        # Read 13-band image using rasterio
        with rasterio.open(img_path) as src:
            image = src.read()  # Shape: (13, H, W)

        # Resize image to target size
        resized_image = torch.zeros((image.shape[0], *self.target_size), dtype=torch.float32)
        for band in range(image.shape[0]):
            resized_image[band] = torch.tensor(
                cv2.resize(image[band], self.target_size, interpolation=cv2.INTER_LINEAR)
            )
        ...
        # Apply FFT to the entire tensor
        #fft_image = torch.fft.fft2(resized_image)
        fft_data = torch.zeros_like(resized_image, dtype=torch.complex64)
        for i in range(resized_image.shape[0]):
            fft_channel = torch.fft.fft2(resized_image[i])
            fft_data[i] = torch.fft.fftshift(fft_channel)

        return fft_data, label

custom_transform = CustomTransforms(rotation=15, hflip_prob=0.5, vflip_prob=0.5)

trainset_fft = FFTDataset(train_path, transform=custom_transform)
valset_fft = FFTDataset(val_path)
testset_fft = FFTDataset(test_path)
print("Unique labels:", set(trainset_fft.labels))
print("Unique labels:", set(trainset_fft.classes))
...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = Net().to(device)
class_weights = torch.tensor([0.9000, 0.9000, 0.9000, 1.0800, 1.0800, 1.3500, 1.0800, 0.9000, 1.0800,
        0.9000]).to(device)
#criterion = nn.CrossEntropyLoss(weight=class_weights)
criterion = nn.CrossEntropyLoss()
#optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay =5e-4, amsgrad=True) 
#optimizer = torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.9, weight_decay=0.0001) #Relu
optimizer = optim.Adam(model.parameters(), lr=0.0001) #Softplus
#optimizer = optim.Adam(model.parameters(), lr=0.01)

log_file = "Eurosat_soft_training_logs144_164_4.csv"
train_losses, val_losses, train_accuracies, val_accuracies = train_model(model, trainloader_fft, validationloader_fft, testloader_fft, 
    criterion, optimizer, num_epochs=1000, log_file_path=log_file)
